...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/medic/13_distillation-for-wikiseealsotitles-with-oak-007'
    ...
    """ metadata label centroid representation """

    batch_size = 1024
    meta_embeddings = []
    mean_meta_repr = m_teacher.lbl_repr.weight.mean(dim=0)

    meta_lbl = block.train.dset.meta['cat_meta'].lbl_meta.T.tocsr()

    for i in tqdm(range(0, meta_lbl.shape[0], batch_size)):
        indptr = meta_lbl.indptr[i:i+batch_size+1]
        a,b = indptr[0], indptr[-1]

        idx = torch.tensor(meta_lbl.indices[a:b])
        meta_repr = m_teacher.lbl_repr(idx)
    ...
    m_student.encoder.set_pretrained_meta_embeddings(meta_embeddings)
