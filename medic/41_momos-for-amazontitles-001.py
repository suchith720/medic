...
os.environ['WANDB_PROJECT']='medic_09-amazontitles'
...
def create_optimizer_and_scheduler(self:XCLearner, num_training_steps: int):
    ...
    for k, p in model.named_parameters():
        if p.requires_grad:
            if "meta_embeddings" not in k and "label_embeddings" not in k: 
                dense.append((k,p))
            else: 
                sparse.append(p)
...
# %% ../nbs/41_momos-for-amazontitles.ipynb 8
@patch
...
# %% ../nbs/41_momos-for-amazontitles.ipynb 9
...
if __name__ == '__main__':
    ...
    output_dir = f'{output_dir}/41_momos-for-amazontitles'

    teacher_dir = '/home/aiscuser/scratch1/medic/data/teacher/LF-AmazonTitles-1.3M/'
    meta_embed_file = '/data/OGB_Weights/LF-AmazonTitles-1.3M/emb_weights.npy'

    """ Load data """
    pkl_file = f'{pkl_dir}/processed/amazontitles_data-lnk_distilbert-base-uncased_oak.pkl'

    if build_block:
        block = XCBlock.from_cfg(data_dir, 'data_lnk', dset='amazontitles', transform_type='oak', tokenizer='distilbert-base-uncased',
                                 padding=True, return_tensors='pt', num_labels=3, num_metadata=3, metadata_name='lnk')
    ...
    block.collator.tfms.tfms[0].n_labels = 3
