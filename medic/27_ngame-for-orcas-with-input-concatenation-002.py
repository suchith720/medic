...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/medic/27_ngame-for-orcas-with-input-concatenation-002'
    tokz = AutoTokenizer.from_pretrained('distilbert-base-uncased')

    """ Load data """
    os.makedirs(f'{pkl_dir}/processed', exist_ok=True)
    ...
    if not os.path.exists(pkl_file):
        ...
        block = XCBlock.from_cfg(data_dir, 'data_meta', dset='orcas', transform_type='xcs', tokenizer='distilbert-base-uncased', 
                                 sampling_features=[('lbl2data',1)], oversample=False, info_column_names=['input_text'], 
                                 sep=None)
    ...
    o = perform_padding(tokz, block.train.dset.data.data_info['input_ids_aug_gpt'], block.train.dset.data.data_info['attention_mask_aug_gpt'])
    block.train.dset.data.data_info['input_ids'],block.train.dset.data.data_info['attention_mask'] = o['input_ids'], o['attention_mask']
    o = perform_padding(tokz, block.test.dset.data.data_info['input_ids_aug_gpt'], block.test.dset.data.data_info['attention_mask_aug_gpt'])
    block.test.dset.data.data_info['input_ids'], block.test.dset.data.data_info['attention_mask'] = o['input_ids'], o['attention_mask']

    o = perform_padding(tokz, block.train.dset.data.lbl_info['input_ids_aug_gpt'], block.train.dset.data.lbl_info['attention_mask_aug_gpt'])
    block.train.dset.data.lbl_info['input_ids'],block.train.dset.data.lbl_info['attention_mask'] = o['input_ids'], o['attention_mask']
    o = perform_padding(tokz, block.test.dset.data.lbl_info['input_ids_aug_gpt'], block.test.dset.data.lbl_info['attention_mask_aug_gpt'])
