# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/34_momos-slm-for-wikiseealsotitles.ipynb.

# %% auto 0
__all__ = ['get_label_remap']

# %% ../nbs/34_momos-slm-for-wikiseealsotitles.ipynb 2
...
from xcai.models.distillation import DTL012,TCH003
...
# %% ../nbs/34_momos-slm-for-wikiseealsotitles.ipynb 7
def get_lbl_centroid(lbl_data, data_rep, batch_size=2048):
    lbl_rep = None
    for i in tqdm(range(0, lbl_data.shape[0], batch_size)):
        data = lbl_data[i:i+batch_size]
        rep = data@data_rep
        rep = rep/data.getnnz(axis=1).reshape(-1, 1)
        rep = torch.tensor(rep, dtype=data_rep.dtype)
...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/medic/34_momos-slm-for-wikiseealsotitles-004'
    teacher_output = '/data/to_b/phi_2_last_token.pt'
    ...
    """ Teacher model """
    data_embed = torch.load(teacher_output).to(torch.float32)
    assert data_embed.shape[0] == block.train.dset.n_data, 'teacher embeddings shape mismatch'

    config = DistilBertConfig()
    config.dim = data_embed.shape[1]

    m_teacher = TCH003(config, n_data=block.train.dset.n_data)
    m_teacher.init_embeddings(data_embed)
    m_teacher.freeze_embeddings()

    lbl_embed = get_lbl_centroid(block.train.dset.data.data_lbl.T, data_embed, batch_size=2048)
    lbl_remap, n_clusters = get_label_remap(lbl_embed, cluster_sz=3)
    ...
    """ Distillation model """
    model = DTL012(DistilBertConfig(), m_student=m_student, m_teacher=m_teacher, bsz=bsz, tn_targ=5000, margin=0.3, tau=0.1, 
