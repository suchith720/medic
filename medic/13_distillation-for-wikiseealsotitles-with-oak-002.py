...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/medic/13_distillation-for-wikiseealsotitles-with-oak-002'
    ...
    idx = np.where(np.logical_and(nnz < 100, nnz > 0))[0]

    """ preparing linker """
    def get_meta_dataset(meta, data_meta, idx):
        data_meta = data_meta[:, idx]
        lbl_meta = meta.lbl_meta[:, idx]
        meta_info = {k:[v[i] for i in idx] for k,v in meta.meta_info.items()}
        return MetaXCDataset('lnk', data_meta, lbl_meta, meta_info)

    fname = '/home/aiscuser/scratch1/outputs/medic/23_ngame-linker-for-wikiseealsotitles-003/linker/train_predictions.pkl'
    with open(fname, 'rb') as file: o = pickle.load(file)
    data_meta = get_pred_sparse(o, block.train.dset.meta['cat_meta'].n_meta)
    block.train.dset.meta['lnk_meta'] = get_meta_dataset(block.train.dset.meta['cat_meta'], data_meta, idx)

    fname = '/home/aiscuser/scratch1/outputs/medic/23_ngame-linker-for-wikiseealsotitles-003/linker/test_predictions.pkl'
    with open(fname, 'rb') as file: o = pickle.load(file)
    data_meta = get_pred_sparse(o, block.test.dset.meta['cat_meta'].n_meta)
    block.test.dset.meta['lnk_meta'] = get_meta_dataset(block.test.dset.meta['cat_meta'], data_meta, idx)
